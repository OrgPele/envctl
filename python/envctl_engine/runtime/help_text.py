...
def _render_general_help() -> str:
    ...
    return "\n".join(
        [
            ...
            "  --no-backend            remove backend service/prep from startup or plan worktree runs",
            "  --no-frontend           remove frontend service/prep from startup or plan worktree runs",
...
COMMAND_HELP_TOPICS: dict[str, CommandHelpTopic] = {
    "start": CommandHelpTopic(
        ...
        flags=(
            ...
            "--no-backend|--no-frontend|--no-deps|--no-infra  remove launch parts from startup/plan runs",
        ...
        examples=(
            ...
            "envctl --trees --no-frontend --no-deps --headless",
        ),
    ...
    "plan": CommandHelpTopic(
        ...
        flags=(
            ...
            "--no-backend|--no-frontend|--no-deps|--no-infra  skip local app/dependency launch pieces",
