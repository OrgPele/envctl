...
5. Compose one complete commit/PR handoff message and pass it inline with `envctl ship -m "<message>"`; do not run `envctl commit` separately in the normal handoff path, and do not write envctl-local commit-message ledger files. Its Verification section must state what your validation actually did and proved, then state any manual checks a human should still run to truly confirm it works, with expected results.
