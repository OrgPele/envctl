...
- If a real subagent/background task tool exists, delegate `ship` there. The worker reports only failures, merge conflicts, failed checks, pending-check timeout, no-checks-reported status, or actionable review comments; return to the shipping lane only when it reports an issue.
