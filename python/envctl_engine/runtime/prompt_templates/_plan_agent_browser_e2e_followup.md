...
2. Extract every manual/human check from the Verification section and try to perform every automatable check yourself through the conventional deployment URL, especially browser or E2E checks.
...
5. If the feature is browser-visible or can be observed through the browser, prove it is visible in the browser and capture evidence.
6. You must fix any issue, regression, or mismatch introduced by the implementation before final handoff, then rerun the relevant checks against the conventional deployment URL and report what each check proved.
