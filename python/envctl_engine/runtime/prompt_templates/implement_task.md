...
- When the implementation is complete and relevant tests are green, prefer `envctl ship --project <current-worktree-name> --json`. It commits, pushes, creates or reuses the PR, and returns the PR URL, current GitHub check state, and merge-conflict details. `ship` does not wait for pending checks by itself, so continue useful local work while CI is pending, but before final handoff watch checks with `gh pr checks --watch` or equivalent until required checks pass. If `ship` is unavailable, fall back to `envctl commit`, `envctl pr`, and `gh pr checks`.
