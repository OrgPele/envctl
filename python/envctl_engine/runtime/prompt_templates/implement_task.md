...
Final output must include: what changed, files changed, tests run, commit status, PR status and URL, and any material assumptions or residual risks. If a PR URL exists, repeat it as the final line of the response when practical.
...
- Do not run manual staging commands such as `git add .` during normal implementation; use `envctl test-focused --ship-on-pass "<message>"` when focused validation has not already passed for the final tree, or `envctl ship -m "<message>"` when it has, and let envctl handle intended files.
...
- During implementation, follow AGENTS.md for the focused validation-and-handoff workflow. Use `envctl test-focused --ship-on-pass "<message>"` as the ideal handoff when focused validation has not already passed for the final tree; if it has already passed, use `envctl ship -m "<message>"` and do not rerun tests. Do not run standalone `envctl test-focused`, repeat a passing focused run, or run `envctl test --all` or other broad local suites before handoff. Full suites are CI-owned; use narrow diagnostic commands only when a focused validation failure needs investigation.
...
## Handoff
- Follow the AGENTS.md ship workflow. Keep one complete commit/PR handoff message ready and pass it inline with `envctl test-focused --ship-on-pass "<message>"` when focused validation has not already passed for the final tree; it validates and then runs the standard ship workflow, including staging intended changes via git add, commit, push, PR create/update, and check reporting. If focused validation already passed for the final tree, use `envctl ship -m "<message>"` and do not rerun tests. Also use `envctl ship -m "<message>"` when the combined command is unavailable or returns actionable fallback instructions. Do not run standalone `envctl test-focused` first or repeat it afterward. Do not run `envctl commit` separately unless `ship` is unavailable, blocked by the environment, or you are intentionally performing a commit-only maintenance operation. Do not write envctl-local commit-message ledger files.
...
- Do not start or deploy a local envctl runtime as the default proof path. The normal implementation lane is: make the code change, then use `envctl test-focused --ship-on-pass "<message>"` as the single envctl local validation-and-handoff command unless focused validation already passed for the final tree, in which case use `envctl ship -m "<message>"`.
- Do not run extra repo test commands as a default proof path. Full/all local suites are not part of the prompt workflow; CI runs them. Use narrow diagnostic commands only after a focused validation failure, and do not rerun tests just to reconfirm after a passing combined command.
