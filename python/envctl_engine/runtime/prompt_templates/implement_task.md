...
- When the implementation is complete and relevant tests are green, prefer `envctl ship --json` from inside the current generated worktree. When running from outside the worktree, use `envctl ship --project <current-worktree-name> --json`. It commits, pushes, opens or reuses the PR, and returns the PR URL, current GitHub check state, `failing_checks`, `pending_checks`, and merge-conflict details. `ship` does not wait for pending checks by itself. When subagents are available, delegate `envctl ship --json` to a background subagent and keep the main implementation thread moving on non-overlapping work. Only return to the shipping lane when the subagent reports a bad result, including commit/push/PR failures, `status: "merge_conflicts"`, failed PR status checks, or actionable review comments. Before final handoff, watch checks with `gh pr checks --watch` or equivalent until required checks pass. If `ship` is unavailable, fall back to `envctl commit`, `envctl pr`, and `gh pr checks`.
