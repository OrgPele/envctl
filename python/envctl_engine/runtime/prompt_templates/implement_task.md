...
- During implementation, prefer `envctl test-plan --project <current-worktree-name> --json` to choose focused tests, then run the listed commands. Run broader validation when the test plan recommends it or the change is cross-cutting/risky.
- When the implementation is complete and the relevant tests are green, prefer `envctl ship --project <current-worktree-name> --json` for final handoff. It reuses envctl commit/PR behavior, reports the PR URL, and returns GitHub status-check state. If `envctl ship` is unavailable or fails for environmental reasons, fall back to `envctl commit`, `envctl pr`, and `gh pr checks`/the GitHub CLI manually.
