...
- When the implementation is complete and the relevant tests are green, commit the work. Prefer `envctl commit --headless` first so the repo-standard commit flow is used without blocking on interactive prompts; if `envctl commit --headless` does not work, fall back to the git CLI and still produce the commit.
