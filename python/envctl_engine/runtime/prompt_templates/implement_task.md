...
- When the implementation is complete and relevant tests are green, prefer `envctl ship --project <current-worktree-name> --json`. It commits, pushes, creates or reuses the PR, and returns the PR URL, current GitHub check state, and merge-conflict details. `ship` does not wait for pending checks by itself. When subagents are available, delegate `envctl ship --project <current-worktree-name> --json` to a background subagent and keep the main implementation thread moving on non-overlapping work while CI is pending. The shipping subagent must report any bad result immediately, including commit/push/PR failures, `status: "merge_conflicts"`, failed checks, or actionable review comments. Before final handoff, watch checks with `gh pr checks --watch` or equivalent until required checks pass. If `ship` is unavailable, fall back to `envctl commit`, `envctl pr`, and `gh pr checks`.
