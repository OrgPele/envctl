...
- When you refer to that follow-up, ground it in the real supported flow:
  ...
  - explain the supported launch surfaces clearly enough that the user does not need to run `envctl --help` to understand them:
    - `--cmux`: envctl opens a cmux surface, launches the selected CLI, and submits the workflow there
    ...
    - `--cmux --opencode` / `--tmux --opencode`: envctl launches OpenCode, submits the rendered prompt body directly, and prepends `/ulw-loop` to the first submitted prompt by default
    ...
    - `--new-session` / `--fresh-session`: create a fresh AI terminal session/surface for the selected worktree instead of reusing or attaching to an existing one
  ...
  - make clear that `opencode` applies to cmux and tmux launcher paths; OMX-managed launches are Codex-only
  ...
  - when you execute envctl launch commands yourself from an AI session, always add `--new-worktree` so you create a fresh implementation worktree with a fresh AI session unless the task explicitly requires reuse
  ...
  - if an existing plan worktree/session may already exist and the user wants another implementation attempt without being prompted, include `--new-worktree` in the shown command
  ...
  - do not surface the internal `--new-worktree` default in the user-facing approval question unless the task specifically requires explaining it
