## Decision
When the current implementation pass finishes, continue implementation if anything remains. If the first implementation pass is complete, use `envctl test-focused --ship-on-pass "<message>"` instead of a separate validation, git add, commit, push, and PR flow.
