## Decision
When the current implementation pass finishes, continue implementation if anything remains. If the first pass is complete, use `envctl test-focused --ship-on-pass "<message>"` instead of separate validation and ship commands.
