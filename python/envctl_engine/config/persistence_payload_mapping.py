...
from envctl_engine.shared.parsing import bool_text


def hydrate_payload_mapping(payload: dict[str, object], base_mapping: dict[str, str]) -> dict[str, str]:
    mapping = dict(base_mapping)
    if _apply_flat_payload(payload, mapping):
        return mapping

    _apply_default_mode(payload, mapping)
    _apply_directories(payload, mapping)
    _apply_ports(payload, mapping)
    _apply_network(payload, mapping)
    _apply_additional_services(payload, mapping)
    _apply_profiles(payload, mapping)
    return mapping
...
def _apply_additional_services(payload: dict[str, object], mapping: dict[str, str]) -> None:
    ...
    for item in additional_services:
        ...
        names.append(name)
        _apply_additional_service(item, mapping, suffix=suffix)
    if names:
...
def _apply_additional_service(item: dict[Any, Any], mapping: dict[str, str], *, suffix: str) -> None:
    ...
    mapping[f"{prefix}MAIN_ENABLE"] = bool_text(bool(item.get("enabled_main", True)))
    mapping[f"{prefix}TREES_ENABLE"] = bool_text(bool(item.get("enabled_trees", True)))
    mapping[f"{prefix}EXPECT_LISTENER"] = bool_text(bool(item.get("expect_listener", True)))
    ...
    if item.get("critical") is not None:
        mapping[f"{prefix}CRITICAL"] = bool_text(bool(item["critical"]))
...
def _apply_profiles(payload: dict[str, object], mapping: dict[str, str]) -> None:
    ...
    for mode in ("main", "trees"):
        ...
        if isinstance(profile, dict):
            _apply_profile(mode=mode, profile=profile, mapping=mapping)


def _apply_profile(*, mode: str, profile: dict[Any, Any], mapping: dict[str, str]) -> None:
    if profile.get("startup_enabled") is not None:
        mapping[f"{mode.upper()}_STARTUP_ENABLE"] = bool_text(bool(profile["startup_enabled"]))
    if profile.get("backend") is not None:
        mapping[f"{mode.upper()}_BACKEND_ENABLE"] = bool_text(bool(profile["backend"]))
    if profile.get("backend_expect_listener") is not None:
        mapping[f"{mode.upper()}_BACKEND_EXPECT_LISTENER"] = bool_text(bool(profile["backend_expect_listener"]))
    if profile.get("frontend") is not None:
        mapping[f"{mode.upper()}_FRONTEND_ENABLE"] = bool_text(bool(profile["frontend"]))
    ...
    for dependency_id, enabled in dependencies.items():
        ...
        if definition is not None:
            mapping[definition.enable_keys_for_mode(mode)[0]] = bool_text(bool(enabled))
