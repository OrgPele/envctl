...
def _run_wizard(*, local_state: LocalConfigState, emit: Callable[..., None] | None = None) -> ConfigWizardResult | None:
    initial_values = managed_values_from_local_state(local_state)
    result = run_config_wizard_textual(local_state=local_state, initial_values=initial_values, emit=emit)
    if result is None:
        return None
