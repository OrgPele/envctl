...
from envctl_engine.startup.execution_plan import apply_project_startup_result_to_session
from envctl_engine.startup.project_execution import execute_project_startup_plan
from envctl_engine.startup.run_reuse_application import resolve_run_reuse_for_session
...
class StartupOrchestrator:
    ...
    def _resolve_run_reuse(self, session: StartupSession) -> int | None:
        return resolve_run_reuse_for_session(
            self,
            session,
            evaluate_run_reuse_fn=evaluate_run_reuse,
            mark_run_reused_fn=mark_run_reused,
            build_planning_dashboard_state_fn=build_planning_dashboard_state,
            resolve_state_repository_fn=resolve_state_repository,
            build_runtime_map_fn=build_runtime_map,
        )
    ...
    def _start_selected_contexts(self, session: StartupSession) -> None:
        execute_project_startup_plan(
            ...
            project_success_message_fn=_project_spinner_success_message,
        )
    ...
    def _record_project_startup(
        ...
        apply_project_startup_result_to_session(session, context, result)
