...
class ResumeProjectRestoreRunner:
    ...
    def restore_project(self, index: int, project: str) -> dict[str, object]:
        ...
        if self.use_project_spinner_group:
            ...
            self.active_spinner.update(message)
        ...
        if self.use_project_spinner_group:
            self.project_spinner_group.update_project(project, f"{prefix} Resolving runtime context...")
        elif self.use_single_spinner:
            self.active_spinner.update(f"{prefix} Resolving runtime context for {project}...")
        ...
        if context is None:
            ...
            if self.use_project_spinner_group:
                self.project_spinner_group.mark_failure(project, f"{prefix} Context not found")
            elif self.use_single_spinner:
                self.active_spinner.update(f"{prefix} Context not found for {project}")
        ...
        missing_project_service_names = [name for name in project_service_names if name in set(self.missing_services)]
        ...
        if existing_requirements is not None and not requirements_reused:
            if self.use_project_spinner_group:
                ...
                self.active_spinner.update(f"{prefix} Releasing previous requirement ports for {project}...")
        ...
        try:
            if requirements_reused and requirements_for_services is not None:
                ...
                if not self.rt._requirements_ready(requirements):  # type: ignore[attr-defined]
                    ...
                    if self.use_project_spinner_group:
                        self.project_spinner_group.mark_failure(project, f"{prefix} Requirements unavailable")
                    elif self.use_single_spinner:
                        self.active_spinner.update(f"{prefix} Requirements unavailable for {project}")
                    project_total = _round_ms(time.monotonic() - project_started)
                    self.rt._emit(  # type: ignore[attr-defined]
                        "resume.restore.project_timing",
                        run_id=self.state.run_id,
            ...
            if self.use_project_spinner_group:
                self.project_spinner_group.update_project(project, f"{prefix} Stopping stale services...")
            elif self.use_single_spinner:
                self.active_spinner.update(f"{prefix} Stopping stale services for {project}...")
            ...
            if self.use_project_spinner_group:
                self.project_spinner_group.update_project(project, f"{prefix} Reserving service ports...")
            elif self.use_single_spinner:
                self.active_spinner.update(f"{prefix} Reserving service ports for {project}...")
            ...
            if self.use_project_spinner_group:
                self.project_spinner_group.update_project(project, f"{prefix} Starting app services...")
            elif self.use_single_spinner:
                self.active_spinner.update(f"{prefix} Starting app services for {project}...")
            ...
            if self.use_project_spinner_group:
                self.project_spinner_group.mark_success(project, f"{prefix} restored")
            elif self.use_single_spinner:
                self.active_spinner.update(f"{prefix} {project} restored")
            project_total = _round_ms(time.monotonic() - project_started)
            self.rt._emit(  # type: ignore[attr-defined]
                "resume.restore.project_timing",
                run_id=self.state.run_id,
            ...
            if self.use_project_spinner_group:
                self.project_spinner_group.mark_failure(project, f"{prefix} Restore failed: {exc}")
            elif self.use_single_spinner:
                self.active_spinner.update(f"{prefix} Restore failed for {project}: {exc}")
            project_total = _round_ms(time.monotonic() - project_started)
            self.rt._emit(  # type: ignore[attr-defined]
                "resume.restore.project_timing",
                run_id=self.state.run_id,
