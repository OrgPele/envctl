...
_DEFAULT_APP_SERVICE_TYPES = {"backend", "frontend"}
...
def _has_explicit_local_system_signal(
    ...
    *,
    ...
    selected_default_services = selected_service_types.intersection(_DEFAULT_APP_SERVICE_TYPES)
...
def _selected_defaults_have_autodetected_system(
    ...
    *,
    ...
    for service_name in sorted(selected_service_types.intersection(_DEFAULT_APP_SERVICE_TYPES)):
...
def _should_skip_no_local_app_system(
    rt: object,
    *,
    context: ProjectContextLike,
    route: Route | None,
    ...
    if route is None or route.command != "plan" or route.flags.get("runtime_scope") != "entire-system":
        return False
    if not selected_service_types or not selected_service_types <= _DEFAULT_APP_SERVICE_TYPES:
        return False
...
def start_project_services(
    ...
    *,
    ...
    if not selected_service_types:
        ...
        return {}
    if _should_skip_no_local_app_system(
        rt,
        context=context,
        route=route,
        mode=effective_mode,
