...
def _route_requested_entire_system(route: Route | None) -> bool:
    if route is None:
        return False
...
def _has_explicit_app_service_signal(rt: Any, *, mode: str, service_name: str) -> bool:
...
def _service_has_resolvable_command(rt: Any, *, service_name: str, project_root: Path, port: int) -> bool:
    try:
...
def _selected_services_are_default_no_system(
    rt: Any,
    *,
    ...
    if not _route_requested_entire_system(route):
        return False
    ...
    if not selected_service_types.issubset({"backend", "frontend"}):
        return False
    if any(
        _has_explicit_app_service_signal(rt, mode=mode, service_name=service_name)
    ...
    return not any(
        _service_has_resolvable_command(
            rt,
...
def start_project_services(
    ...
    *,
    ...
    if not selected_service_types:
        ...
        return {}
    no_system_selected_services = _selected_services_are_default_no_system(
        rt,
        context=context,
        mode=effective_mode,
        route=route,
    ...
    if no_system_selected_services:
