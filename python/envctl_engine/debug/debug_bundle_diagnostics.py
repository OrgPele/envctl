...
def write_diagnostics(staging_dir: Path) -> None:
    ...
    for item in startup_timeline:
        ...
        if event_name in {"service.bootstrap.phase", "service.attach.phase"}:
            ...
            target_totals = (
                service_bootstrap_totals if event_name == "service.bootstrap.phase" else service_attach_totals
            )
            target_totals[key] = round(target_totals.get(key, 0.0) + duration_ms, 2)
            slowest_components.append(
                {
                    "kind": "service_bootstrap_phase"
                    if event_name == "service.bootstrap.phase"
                    else "service_attach_phase",
                    "project": project,
        ...
        if event_name == "requirements.adapter":
            ...
            for stage_name, raw_duration in stage_map.items():
                ...
                try:
                    ...
                    continue
                requirements_stage_totals[stage_key] = round(
                    requirements_stage_totals.get(stage_key, 0.0) + duration, 2
                )
    ...
    if (
        startup_window_first is not None
        and startup_window_last is not None
        and startup_window_last >= startup_window_first
    ):
