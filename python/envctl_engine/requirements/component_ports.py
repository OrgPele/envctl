...
def component_internal_resource_ports(component_data: Mapping[str, object]) -> dict[str, int]:
    resources = component_resource_ports(component_data, include_synthetic=True)
    requested = positive_int(component_data.get("requested"))
    if requested is not None:
        resources.setdefault("requested", requested)
    return resources
