...
from envctl_engine.requirements.component_ports import component_internal_resource_ports, positive_int
...
def project_env(*, runtime, context, requirements, route=None) -> dict[str, str]:
    ...
    resources = component_internal_resource_ports(component)
    port = (
        positive_int(resources.get("db"))
        or positive_int(resources.get("primary"))
        or positive_int(component.get("final"))
        or positive_int(resources.get("requested"))
        or int(context.ports["db"].final)
    )
    public_port = positive_int(resources.get("api")) or _context_port(context, "supabase_api") or port
