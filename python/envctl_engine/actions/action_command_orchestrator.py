...
class ActionCommandOrchestrator:
    ...
    def _main_repo_root_for_worktree(self, worktree_root: Path, *, trees_dir_name: str | None = None) -> Path | None:
        normalized_trees_dir = (
            str(trees_dir_name or getattr(self.runtime.raw_runtime.config, "trees_dir_name", "trees"))
            .strip()
            .rstrip("/")
        )
        ...
        if not common_dir_raw:
            return top_level
        common_dir = (
            (worktree_root / common_dir_raw).resolve()
...
""".strip()
        ...
        *,
        ...
        for index, record in enumerate(records):
            ...
            if record.status == "success":
                print(
                    f"{ActionCommandOrchestrator._render_migrate_symbol('✓', status='success', use_color=use_color)} "
                    f"migrate succeeded for {project_name}"
                )
                continue
    ...
    def _migrate_failure_hint_lines(error_output: str) -> list[str]:
        ...
        return [
            ...
            "hint: APP_ENV_FILE is exported when an env file is found, and running projects reuse current "
            "dependency URLs when available.",
        ]
