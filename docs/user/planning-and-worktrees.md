...
- `$envctl-create-plan-auto-opencode` writes the plan, derives the selector, then runs `envctl --plan <selector> --tmux --opencode --entire-system --headless --new-worktree`; OpenCode prepends `/ulw-loop` by default and reports `handoff_pending` with attach guidance when the session is alive but prompt submission did not complete.
