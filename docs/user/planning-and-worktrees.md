...
- `$envctl-create-plan-auto-codex` writes `todo/plans/<category>/<slug>.md`, derives `<category>/<slug>` from that path, chooses a recommended Codex cycle count from `0` through `8`, records plan-selected launch-scope flags, then runs `ENVCTL_PLAN_AGENT_CODEX_CYCLES=<recommended> envctl --plan <selector> --cmux <launch-scope-flags> --headless --new-session`; prompt/static-only plans should use `--no-infra`, while `--entire-system` is reserved for plans that need runtime services, browser validation, or integration coverage. Use `--tmux` only when cmux is unavailable or explicitly requested.
- `$envctl-create-plan-auto-opencode` writes the plan, derives the selector, records plan-selected launch-scope flags, then runs `envctl --plan <selector> --cmux --opencode <launch-scope-flags> --headless --new-session`; prompt/static-only plans should use `--no-infra`, while `--entire-system` is reserved for plans that need runtime services, browser validation, or integration coverage. OpenCode prepends `/ulw-loop` by default, and `--no-ulw-loop` removes it for a one-off plain prompt.
