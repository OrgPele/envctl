...
- `$envctl-create-plan-auto-codex` writes the same kind of plan, derives `<category>/<slug>` from the plan file path, then runs `ENVCTL_PLAN_AGENT_CODEX_CYCLES=2 envctl --plan <selector> --tmux --entire-system --headless --tmux-new-session`.
...
- `$envctl-create-plan-auto-omx` writes the plan, derives `<selector>`, then runs `envctl --plan <selector> --omx --ralph --entire-system --headless --tmux-new-session`. OMX-managed launches are Codex-only: optional `/goal` framing is submitted first, Ralph wraps the initial prompt, and envctl can queue the same Codex follow-up cycles used by plain Codex launches.
