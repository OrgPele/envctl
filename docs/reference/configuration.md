...
| `ENVCTL_PLAN_AGENT_PRESET` | `implement_task` | Prompt preset name submitted after the AI CLI starts. OpenCode launches send `/<preset>`. Codex resolves the preset from the envctl-managed Codex prompt file and submits that prompt body directly. `implement_plan` remains available as a backward-compatible preset. |
...
| Variable | Default | Purpose |
| --- | --- | --- |
| `ENVCTL_EXPERIMENTAL_CODEX_SKILLS` | `false` | Enable the experimental `install-prompts --with-codex-skills` path that mirrors envctl Codex presets as explicit-only skills under `~/.codex/skills/envctl-*`. |
