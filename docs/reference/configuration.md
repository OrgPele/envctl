...
- Superset receives a single public CLI prompt. For Codex launches with goal framing enabled, envctl prefixes that prompt with `/goal ...`; Codex cycle queueing, screen polling, tab renaming, key sending, cmux surfaces, and dashboard review tabs are not supported through Superset public CLI.
