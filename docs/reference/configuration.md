...
| `ENVCTL_PLAN_AGENT_PRESET` | `implement_task` | Prompt preset name submitted after the AI CLI starts. OpenCode launches send `/<preset>`. Codex uses the preset's configured submission mode: envctl's legacy presets still use the old slash-command path until migrated, while direct-submission presets such as `ship_release` send the prompt body from the envctl-managed Codex prompt file. `implement_plan` remains available as a backward-compatible preset. |
