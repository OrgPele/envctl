...
- `$envctl-create-plan-auto-omx` uses `--omx --ralph`; Codex `/goal` framing is submitted first when enabled, Ralph wraps the initial prompt, and envctl can queue the same Codex follow-up cycle workflow used by plain Codex TUI launches
