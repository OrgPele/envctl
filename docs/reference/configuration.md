...
| `ENVCTL_PLAN_AGENT_CODEX_CYCLES` | `2` | Codex-only queued workflow count for the post-`--plan` launcher. The default `2` queues a first follow-up telling Codex to commit, push, open or update the PR, and wait for GitHub status checks before the final `continue_task` -> `implement_task` -> `finalize_task` round, then queues `$browser-use` E2E and PR-review-comment follow-ups after finalization. `0` submits the single implementation prompt and queues the `$browser-use` E2E plus PR-review-comment follow-ups for Codex/OMX surfaces. `1` queues `implement_task`, `finalize_task`, `$browser-use`, and the PR-review-comment follow-up. Higher values keep that first follow-up, use commit/push-only follow-ups for intermediate rounds, and reserve `finalize_task` plus the queued `$browser-use` E2E and PR-review-comment follow-ups for the final round. OpenCode ignores this setting. Envctl only appends prompt commands/messages in this mode; it does not execute those shell commands itself. |
...
- the queued cycle workflow is active only for Codex and only when `ENVCTL_PLAN_AGENT_CODEX_CYCLES` is a positive integer; the final `$browser-use` E2E and PR-review-comment follow-ups are queued for Codex/OMX surfaces even when the cycle count is `0`
- invalid or negative values are ignored and the launcher stays on the single implementation prompt plus `$browser-use` E2E and PR-review-comment follow-up workflow
