...
| Variable | Default | Purpose |
| --- | --- | --- |
| `ENVCTL_PLAN_AGENT_TERMINALS_ENABLE` | `false` | Enable post-`--plan` agent launch for newly created worktrees and explicit launch-enabled `--import` runs. Plain `--import` without a launch flag only prepares or updates the worktree. The default workspace-backed transport is cmux. |
| `ENVCTL_PLAN_AGENT_SURFACE_TRANSPORT` | `cmux` | Workspace-backed launch transport for default `--plan` and launch-enabled `--import` runs. Accepted values are `cmux` and `superset`; explicit `--cmux`, `--tmux`, and `--omx` route flags override this setting. `--opencode` selects the CLI and only falls back to tmux when no surface transport flag is provided. |
