...
| `ENVCTL_PLAN_AGENT_CODEX_CYCLES` | `1` | Codex-only queued workflow count for the post-`--plan` launcher. The default `1` queues `implement_task` plus `finalize_task`. `0` keeps the existing one-shot preset launch. `2` adds a first follow-up telling Codex to commit, push, and open or update the PR before the final `continue_task` -> `implement_task` -> `finalize_task` round. In that PR step, the intended behavior is to open the PR or edit its title/body, not to post a PR comment unless the user explicitly asks for one. Higher values keep that first follow-up, use commit/push-only follow-ups for intermediate rounds, and reserve `finalize_task` for the final round. OpenCode ignores this setting. Envctl only appends prompt commands/messages in this mode; it does not execute those shell commands itself. |
