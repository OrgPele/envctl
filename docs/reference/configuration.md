...
| `ENVCTL_PLAN_AGENT_CODEX_GOAL_ENABLE` | `true` | Toggle best-effort Codex `/goal` submission before the first plan-agent implementation prompt. Applies only to Codex launches, including OMX/Ralph; OpenCode is effectively disabled regardless of this value. Use `false` or `--no-codex-goal` if a Codex version changes the interactive `/goal` command. |
...
- Codex plan-agent launches submit `/goal ...` before the implementation prompt by default; OMX/Ralph ordering is goal first, then the `$ralph`-wrapped prompt
