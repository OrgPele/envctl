...
- `ENVCTL_SHIP_PR_LABEL_ENABLE=true` opts in to programmatic PR labeling during `envctl ship`; the label name defaults to `envctl-shipped-pr` and can be changed with `ENVCTL_SHIP_PR_LABEL`
