...
class PromptInstallSupportTemplatesTests(PromptInstallSupportTestCase):
    ...
    def test_renderers_produce_expected_target_shapes(self) -> None:
        ...
        self.assertIn("instead of separate validation and ship commands", intermediate_prompt)
        ...
        self.assertIn("separate validation and ship commands", review_comments_prompt)
        ...
        self.assertIn("use `envctl ship` only as fallback", plan_prompt.body)
        self.assertIn("If ship returns `deployment_url`, that URL is the deployed website and must be tested thoroughly E2E", plan_prompt.body)
