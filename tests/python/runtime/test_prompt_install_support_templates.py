...
class PromptInstallSupportTemplatesTests(PromptInstallSupportTestCase):
    ...
    def test_create_plan_auto_templates_lock_launch_commands(self) -> None:
        ...
        for preset, contract in expected.items():
            with self.subTest(preset=preset):
                ...
                self.assertIn("Manual / real-world check", body)
    ...
    def test_create_plan_template_requires_browser_e2e_decision(self) -> None:
        ...
        self.assertIn("Manual / real-world check", body)
        self.assertIn("_plan_agent_browser_e2e_followup.md", body)
        self.assertIn("shipped `deployment_url`", body)
        self.assertIn("Rollout / verification", body)
    ...
    def test_renderers_produce_expected_target_shapes(self) -> None:
        ...
        self.assertIn("it validates and then runs the standard ship workflow", codex)
        ...
        self.assertIn("if it has already passed, use `envctl ship -m \"<message>\"` and do not rerun tests", codex)
        self.assertIn("Full suites are CI-owned", codex)
        ...
        self.assertIn("If ship returns a non-empty `deployment_url`", codex)
        self.assertIn("treat it as the deployed website and test it thoroughly E2E", codex)
        ...
        self.assertIn("repeat it as the final line of the response when practical", codex)
        ...
        self.assertIn("PR URL if one exists", continue_prompt.body)
        self.assertIn("repeat it as the final line of the response when practical", continue_prompt.body)
        ...
        self.assertIn("PR URL if one exists", finalize_prompt.body)
        ...
        self.assertIn("If focused validation already passed for the final tree, use `envctl ship -m \"<message>\"`", finalize_prompt.body)
        self.assertIn("Do not run `envctl test --all`, broad local suites, or extra validation before handoff", finalize_prompt.body)
        self.assertIn(
            "include `--project <current-worktree-name>` on the combined command when that command is needed",
        ...
        self.assertIn("If ship returns a non-empty `deployment_url`", finalize_prompt.body)
        self.assertIn("treat it as the deployed website and test it thoroughly E2E", finalize_prompt.body)
        ...
        self.assertIn("it validates and then runs the standard ship workflow", finalize_prompt.body)
        ...
        self.assertIn("If a PR URL exists after handoff", intermediate_prompt)
        self.assertIn('envctl test-focused --ship-on-pass "<message>"', intermediate_prompt)
        self.assertIn('Use `envctl ship -m "<message>"` when focused validation already passed or fallback is needed', intermediate_prompt)
        self.assertIn("it validates and then runs the standard ship workflow", intermediate_prompt)
        self.assertIn("Do not run standalone `envctl test-focused`, repeat passing tests, or run `envctl test --all`", intermediate_prompt)
        self.assertIn("Do not substitute localhost validation for deployed PR URL validation", intermediate_prompt)
        self.assertIn("If ship returns a non-empty `deployment_url`", intermediate_prompt)
        self.assertIn("treat it as the deployed website and test it thoroughly E2E", intermediate_prompt)
        ...
        self.assertIn("If focused validation already passed for the final tree", intermediate_prompt)
        ...
        self.assertIn("If a PR URL exists after handoff", first_cycle_prompt)
        self.assertIn('envctl test-focused --ship-on-pass "<message>"', first_cycle_prompt)
        self.assertIn('use `envctl ship -m "<message>"` and do not rerun tests', first_cycle_prompt)
        self.assertIn("it validates and then runs the standard ship workflow", first_cycle_prompt)
        self.assertIn("Do not substitute localhost validation for deployed PR URL validation", first_cycle_prompt)
        self.assertIn("If ship returns a non-empty `deployment_url`", first_cycle_prompt)
        self.assertIn("treat it as the deployed website and test it thoroughly E2E", first_cycle_prompt)
        ...
        self.assertIn("If a PR URL exists after handoff", review_comments_prompt)
        self.assertIn('envctl test-focused --ship-on-pass "<message>"', review_comments_prompt)
        self.assertIn('use `envctl ship -m "<message>"` and do not rerun tests', review_comments_prompt)
        ...
        self.assertIn("If ship returns a non-empty `deployment_url`", review_comments_prompt)
        self.assertIn("treat it as the deployed website and test it thoroughly E2E", review_comments_prompt)
        ...
        self.assertIn("If the preceding `envctl test-focused --ship-on-pass` or `envctl ship` output returned a non-empty `deployment_url`", browser_e2e_prompt)
        self.assertIn("It is the deployed website and must receive the thorough browser E2E pass.", browser_e2e_prompt)
        ...
        self.assertIn("the original plan's Manual / real-world check section", browser_e2e_prompt)
        self.assertIn("PR URL if one exists", browser_e2e_prompt)
        self.assertIn("repeat it as the final line when practical", browser_e2e_prompt)

        merge_prompt = _load_template("merge_implementation_branches").body
        self.assertIn("PR URL if one exists", merge_prompt)
        self.assertIn("repeat it as the final line of the response when practical", merge_prompt)
        self.assertIn("do not stop at a page-load smoke test", browser_e2e_prompt)
        ...
        self.assertIn('Separately record the validation lane: `envctl test-focused --ship-on-pass "<message>"` by default when focused validation has not already passed for the final tree', plan_prompt.body)
        self.assertIn('if focused validation already passed, use `envctl ship -m "<message>"` and do not rerun tests', plan_prompt.body)
        ...
        self.assertIn("Use `envctl ship` only after focused validation already passed or as fallback", plan_prompt.body)
        self.assertIn("If ship returns `deployment_url`, that URL is the deployed website and `_plan_agent_browser_e2e_followup.md` must use the `$browser` skill to test it thoroughly E2E", plan_prompt.body)
        self.assertIn("Include a concrete Manual / real-world check", plan_prompt.body)
        self.assertIn("for UI work, name the route/page, section, controls, user actions, expected visible result", plan_prompt.body)
    ...
    def test_prompt_templates_no_longer_reference_changelog_backed_commit_defaults(self) -> None:
        ...
        for prompt in (implement_prompt, continue_prompt, merge_prompt):
            with self.subTest(prompt=prompt.name):
                ...
                self.assertIn("do not rerun tests", prompt.body)
                self.assertIn("Full suites are CI-owned", prompt.body)
                self.assertIn("including staging intended changes via git add", prompt.body)
                self.assertIn("standard ship workflow", prompt.body)
