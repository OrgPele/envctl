...
class EngineRuntimeRequirementsStartupTests(_EngineRuntimeRealStartupTestCase):
    ...
    def test_plan_startup_skips_default_services_when_no_local_app_system_resolves(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = Path(tmpdir) / "repo"
            runtime = Path(tmpdir) / "runtime"
            (repo / ".git").mkdir(parents=True, exist_ok=True)
            ...
            self.assertIn("No local app system is configured for this repo/worktree", out.getvalue())
            self.assertEqual(fake_runner.start_background_calls, [])
    ...
    def test_explicit_service_enablement_still_fails_when_no_command_resolves(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = Path(tmpdir) / "repo"
            runtime = Path(tmpdir) / "runtime"
            (repo / ".git").mkdir(parents=True, exist_ok=True)
