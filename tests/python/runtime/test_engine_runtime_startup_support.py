...
class EngineRuntimeStartupSupportTests(unittest.TestCase):
    ...
    def test_auto_resume_start_enabled_respects_disabling_flags(self) -> None:
        ...
        self.assertFalse(auto_resume_start_enabled(Route(command="start", mode="main", raw_args=[], passthrough_args=[], projects=[], flags={"no_resume": True})))
    ...
    def test_contexts_and_duplicate_detection(self) -> None:
        runtime = SimpleNamespace(port_planner=SimpleNamespace(plan_project_stack=lambda name, index: {"backend": index}))
    ...
    def test_sanitize_legacy_resume_state_and_set_plan_port_helpers(self) -> None:
        ...
        state = RunState(
            run_id="run-1",
            mode="main",
            services={"Main Backend": ServiceRecord(name="Main Backend", type="backend", cwd=".", pid=123, listener_pids=[1])},
        )
    ...
    def test_discover_projects_and_reserve_project_ports(self) -> None:
        ...
        runtime = SimpleNamespace(
            ...
            port_planner=SimpleNamespace(
                plan_project_stack=lambda name, index: {"backend": PortPlan(project=name, requested=8000 + index, assigned=8000 + index, final=8000 + index, source="fixed")},
