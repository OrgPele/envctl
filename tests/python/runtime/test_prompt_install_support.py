...
class PromptInstallSupportTests(unittest.TestCase):
    ...
    def test_renderers_produce_expected_target_shapes(self) -> None:
        ...
        self.assertIn("If the target worktree contains archived task files matching `OLD_TASK_*.md`", review_worktree_prompt.body)
        self.assertIn("lowest-numbered archived task file", review_worktree_prompt.body)
        self.assertIn("Do not start with broad repo exploration before reading the original task file", review_worktree_prompt.body)
        self.assertIn("use that bundle as the primary review guide", review_worktree_prompt.body)
        self.assertNotIn("unless `$ARGUMENTS` overrides that target", review_worktree_prompt.body)
