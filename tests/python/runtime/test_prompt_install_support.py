...
class PromptInstallSupportTests(unittest.TestCase):
    ...
    def test_renderers_produce_expected_target_shapes(self) -> None:
        ...
        self.assertIn("When subagents are available, delegate `envctl ship --project <current-worktree-name> --json`", codex)
        self.assertIn("The shipping subagent must report any bad result immediately", codex)
        ...
        self.assertIn("The shipping subagent must surface merge conflicts", finalize_prompt.body)
        ...
        self.assertIn("then prefer `envctl ship --project <current-worktree-name> --json`", intermediate_prompt)
