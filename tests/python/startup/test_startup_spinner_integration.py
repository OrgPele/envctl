...
class StartupSpinnerIntegrationTests(unittest.TestCase):
    ...
    def test_restart_configured_missing_backend_starts_without_prior_service_record(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            repo = root / "repo"
            runtime = root / "runtime"
            ...
            engine._release_requirement_ports = (  # type: ignore[method-assign]
                lambda _requirements: self.fail("requirements should be preserved for configured missing backend restart")
            )
            ...
            self.assertEqual(terminate_calls, [set()])
            run_state = captured_state["state"]
            self.assertEqual(run_state.services["Main Frontend"].pid, 11111)
            self.assertEqual(run_state.services["Main Backend"].pid, 22222)
            self.assertEqual(run_state.metadata["dashboard_project_configured_services"], {"Main": ["backend", "frontend"]})
