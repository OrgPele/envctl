...
class SupabaseFakeRunner:
    ...
    def run(self, cmd, *, cwd=None, env=None, timeout=None, process_started_callback=None):  # noqa: ANN001
        ...
        if len(command) >= 4 and command[:2] == ["docker", "compose"] and "-f" in command:
            ...
            if compose_args[:3] == ["ps", "--format", "json"] and len(compose_args) >= 4:
                service_name = compose_args[3]
                rc = self.compose_json_returncode.get(service_name, 0)
                stderr = self.compose_json_stderr.get(service_name, "")
                stdout = self.compose_json_stdout.get(service_name, "")
    ...
    def start(self, cmd, *, cwd=None, env=None, stdout_path=None, stderr_path=None):  # noqa: ANN001
        ...
        return SupabaseFakeComposeProcess()
...
class SupabaseFlakyHealthRunner:
    def __init__(self) -> None:
