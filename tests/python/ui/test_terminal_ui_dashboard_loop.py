...
class DashboardLoopTests(unittest.TestCase):
    def test_dashboard_broad_group_dashboard_loop_entry_maps_to_tail_group(self) -> None:
        state = RunState(run_id="run-broad-0", mode="trees")
        runtime = _RuntimeStub(state)
        runtime.env = {"ENVCTL_DEBUG_PLAN_BROAD_GROUP": "dashboard_loop_entry"}
        seen_raw: list[str] = []

        def handle_command(raw: str, current: RunState, _rt: object):  # noqa: ANN001
            seen_raw.append(raw)
        ...
        with patch("envctl_engine.ui.dashboard.terminal_ui.RuntimeTerminalUI._can_interactive_tty", return_value=True):
            code = run_dashboard_command_loop(
                ...
                input_provider=lambda _prompt: self.fail("input provider should not be used"),
            )

        self.assertEqual(code, 0)
        self.assertEqual(seen_raw, ["t"])
        self.assertTrue(
            any(
                event.get("event") == "startup.debug_broad_group"
                and event.get("group") == "dashboard_loop_entry"
                and event.get("mapped_tail_group") == "dashboard_entry"
                for event in runtime.events
            )
        )

    def test_dashboard_tail_group_dashboard_entry_injects_test_command(self) -> None:
        state = RunState(run_id="run-tail-0", mode="trees")
        runtime = _RuntimeStub(state)
        ...
        self.assertTrue(
            any(
                event.get("event") == "startup.debug_tail_group"
                and event.get("group") == "dashboard_entry"
                and event.get("action") == "inject_command"
                for event in runtime.events
            )
        )


    def test_debug_postloop_direct_selector_context_uses_direct_textual_selector(self) -> None:
        state = RunState(run_id="run-postloop-context", mode="trees")
        runtime = _RuntimeStub(state)
        runtime.env = {
            "ENVCTL_DEBUG_PLAN_POSTLOOP_GROUP": "direct_selector",
            "ENVCTL_DEBUG_PLAN_SELECTOR_GROUP": "context",
        }

        with (
            patch("envctl_engine.ui.dashboard.terminal_ui.RuntimeTerminalUI._can_interactive_tty", return_value=True),
            patch("envctl_engine.ui.textual.screens.selector.select_grouped_targets_textual") as direct_selector,
        ):
            direct_selector.return_value = type("Sel", (), {"cancelled": True})()
            code = run_dashboard_command_loop(
                state=state,
                runtime=runtime,
                handle_command=lambda raw, current, _rt: (False, current),
                sanitize=lambda value: value,
                input_provider=lambda _prompt: self.fail("input provider should not be used"),
            )

        self.assertEqual(code, 0)
        direct_selector.assert_called_once()

    def test_debug_postloop_direct_selector_backend_forces_preflight_direct_path(self) -> None:
        state = RunState(run_id="run-postloop-backend", mode="trees")
        runtime = _RuntimeStub(state)
        runtime.env = {
            "ENVCTL_DEBUG_PLAN_POSTLOOP_GROUP": "direct_selector",
            "ENVCTL_DEBUG_PLAN_SELECTOR_GROUP": "backend",
        }

        def _select_grouped_targets(**_kwargs):
            self.assertEqual(runtime.env.get("ENVCTL_DEBUG_PLAN_TTY_COMMON_GROUP"), "preflight")
            return type("Sel", (), {"cancelled": True})()

        runtime._select_grouped_targets = _select_grouped_targets  # type: ignore[attr-defined]
        ...
        self.assertEqual(code, 0)
        self.assertNotIn("ENVCTL_DEBUG_PLAN_TTY_COMMON_GROUP", runtime.env)
