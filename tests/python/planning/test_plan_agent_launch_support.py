...
class PlanAgentLaunchSupportTests(unittest.TestCase):
    ...
    def test_plan_agent_launch_intent_option_matrix_covers_transport_flags(self) -> None:
        cases = (
    ...
    def test_plan_agent_launch_config_uses_shared_transport_intent(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = Path(tmpdir) / "repo"
            runtime = Path(tmpdir) / "runtime"
            repo.mkdir(parents=True, exist_ok=True)
