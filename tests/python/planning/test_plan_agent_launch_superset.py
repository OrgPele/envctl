...
class PlanAgentLaunchSupersetTests(PlanAgentLaunchSupportTestCase):
    ...
    def test_superset_project_launch_uses_public_workspace_create_cli_and_open(self) -> None:
        ...
        self.assertTrue(payload["goal"].startswith("Implement MAIN_TASK.md end-to-end in this worktree."))
        self.assertNotIn("a.md", payload["goal"])
        self.assertNotIn("Flow:", payload["goal"])
        self.assertIn('envctl ship -m "<message>"', payload["goal"])
        self.assertLessEqual(len(payload["goal"]), 180)
