...
class PlanAgentLaunchWorkflowBuildTests(PlanAgentLaunchSupportTestCase):
    ...
    def test_browser_e2e_followup_requires_main_task_browser_visible_validation_and_fix_loop(self) -> None:
        ...
        self.assertIn("URL already published in the generated PR or handoff message", prompt)
        self.assertIn("frontend/app URL that was given to the reviewer", prompt)
        self.assertIn("Do not start services", prompt)
        self.assertIn("If the generated PR/handoff does not contain a usable browser URL", prompt)
        ...
        self.assertIn("try to perform every automatable check yourself through that URL", prompt)
        ...
        self.assertNotIn("envctl --entire-system --headless", prompt)
        self.assertNotIn("envctl list-targets --json", prompt)
        self.assertNotIn("envctl endpoints", prompt)
        self.assertNotIn("envctl qa-user", prompt)
        self.assertNotIn("envctl playwright", prompt)
