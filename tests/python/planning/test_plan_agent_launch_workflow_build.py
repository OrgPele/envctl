...
class PlanAgentLaunchWorkflowBuildTests(PlanAgentLaunchSupportTestCase):
    ...
    def test_browser_e2e_followup_requires_main_task_browser_visible_validation_and_fix_loop(self) -> None:
        ...
        self.assertIn("latest commit message", prompt)
        self.assertIn("frontend/app URL that was given to the reviewer", prompt)
        self.assertIn("Prefer the published URL when it is reachable", prompt)
        self.assertIn("If the published URL is missing, stale, or unreachable", prompt)
        self.assertIn("restart or resolve the envctl target yourself", prompt)
        ...
        self.assertIn("envctl --entire-system --headless", prompt)
